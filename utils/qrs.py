...
def kteager_detect(raw, slice_period=None, tr_period=None, filt_emg=False, filt_kteo=True, picks='pca', l_freq=5, h_freq=15, filter_length=0.04):
    ...
    if picks == 'mean':
        ...
        ecg_data = copy.deepcopy(raw).pick('eeg', exclude='bads').get_data(picks='eeg', reject_by_annotation='NaN')
    ...
    if picks == 'pca':
        ...
        ecg_data = (U[:, :3]*S[None, :3]).sum(dim=-1)  # #timepoints, 1
...
def qrs_correction(ecg_event, raw, ecg_signal, n_events_min, max_heart_rate=160, min_heart_rate=40, new_event_idx=998, corr_thres=[0.8, 0.4, 5], amplitude_thres=0.5, local_maxima=None):
    """
    ...
    Parameters:
        ...
        - n_events_min: Minimum number of events required for the correction to be successful. used for choosing corr_thres.
        ...
        - corr_thres: Threshold for the Pearson correlation coefficient to consider an event as safe. A list of threshold generated by np.linspace(*corr_thres) would be tried, from high to low.
    ...
    if local_maxima is None:
        msk = (ecg_signal[1:-1] > ecg_signal[:-2]) & (ecg_signal[1:-1] > ecg_signal[2:])
        local_maxima = np.where(msk)[0] + 1
    ...
    med = max(60, med)    # note that it is possible that false positive are too many, leading to a very low median, which is not desired.
    ...
    if len(safe_windows) < ecg_event.shape[0] // 10:
        for ev in ecg_event[:,0]:
            ...
            if start >= 0 and end <= raw._data.shape[1]:
                ...
                edge_event.append(ev)
    safe_event = safe_event[np.array([ev not in edge_event for ev in safe_event])]
                
    # remove too high or too low amplitude windows. high amplitude windows are more dangerous here, as bulk movement could distort the tmplate
    safe_windows = np.array(safe_windows)
    amplitude = np.max(np.abs(safe_windows), axis=1)
    amp_med = np.median(amplitude)
    amp_mad = np.median(np.abs(amplitude - amp_med))
    amp_mask = np.ones_like(amplitude, dtype=bool)
    mad_num = 1
    while True:
        new_mask = (amplitude > amp_med - mad_num * amp_mad) & (amplitude < amp_med + mad_num * amp_mad)
        if np.abs(np.mean(new_mask)-amplitude_thres) < np.abs(np.mean(amp_mask)-amplitude_thres):
            amp_mask = new_mask
            mad_num += 1
        else:
            break

    safe_windows = safe_windows[amp_mask]
    tmplt = np.mean(safe_windows, axis=0).squeeze()
    peak_amplitudes = ecg_signal[local_maxima]
    msk = peak_amplitudes > np.max(tmplt) * amplitude_thres
    local_maxima = local_maxima[msk]
    # safe_event = safe_event[amp_mask]
    ...
    def align_with_template(event_time_list, corr_thres_list=np.array([0.8, 0.7, 0.6, 0.5, 0.4])):  
        event_time_list = np.array(event_time_list, dtype=np.int64)
        old_corr = 0    # for recording the last correlation value=
        old_event = None
        new_event_list_by_thres = [[] for _ in corr_thres_list]
        for ev in event_time_list:
            ...
            window_arr = np.stack([ecg_signal[pos-half_win_range:pos+half_win_range+1] for pos in win_pos_list])    # only consider local maxima as possible events
            corr = pearson_corr(window_arr, tmplt)
            
            maxcorr = np.max(corr)
            if maxcorr < corr_thres_list[-1]:
            ...
            if old_event is not None:
                ...
                if best_pos + raw.first_samp - old_event[0] < low_thres:
                    if maxcorr < old_corr:
                        continue
                else:   # delayed addition to the new_event_list, for the case that the modified event is too close to the previous one
                    new_event_list_by_thres[np.argmax(maxcorr>corr_thres_list)].append(old_event)
                  
            old_corr = maxcorr
            old_event = [best_pos+raw.first_samp, 0, new_event_idx]
        
        if old_event is not None:
            new_event_list_by_thres[np.argmax(maxcorr>corr_thres_list)].append(old_event) 
        
        new_event_list = []
        for new_event_list_curr in new_event_list_by_thres:    
            new_event_list = new_event_list + new_event_list_curr
            if len(new_event_list) >= n_events_min:
                break
        else:
            raise RuntimeError(f"Could not find enough heartbeats with even correlation threshold {corr_thres_list[-1]}. Please check the data and template calculation for the subject.")
        
        return np.unique(np.array(new_event_list), axis=0)
    
    new_event = align_with_template(event_timing, corr_thres_list=np.linspace(*corr_thres))
    new_event = np.unique(new_event,axis=0)
    new_event = new_event[np.argsort(new_event[:, 0])]
    spurious_event = copy.deepcopy(new_event)
    ...
    for fn_pos in fn_pos_list:
        ...
        fn = (np.arange(1, hb_in_between) * fn_length + spurious_event[fn_pos][0]).astype(np.int64)
        fn_list.append(fn)
    if spurious_event[0, 0] - raw.first_samp > thres:
        missing_events = np.floor((spurious_event[0, 0] - raw.first_samp) / med)
        fn = spurious_event[0, 0] - (np.arange(missing_events)+1) * med
        fn_list.append(fn)
    if spurious_event[-1, 0] - raw.first_samp + thres < raw._data.shape[1]:
        missing_events = np.floor((raw._data.shape[1] - spurious_event[-1, 0]) / med)
        fn = spurious_event[-1, 0] + (np.arange(missing_events)+1) * med
        fn_list.append(fn)
    ...
    if len(fn_list) > 0:
        ...
        spurious_event = np.concatenate([spurious_event, fn_event])
